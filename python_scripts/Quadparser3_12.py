...
ref_seq=[]
line= (ref_seq_fh.readline()).strip() # moving the file object handle to the next line
chr= re.sub('^>', '', line)
...
while True:
    ...
    if args.noreverse is False:
        for m in re.finditer(psq_re_r, ref_seq):
            ...
            gquad_list.append([chr, m.start(), m.end(), quad_id, len(m.group(0)), '-', m.group(0)])
    chr= re.sub('^>', '', line) # when line does start with >, get the chr name again
    ref_seq= []
    line= (ref_seq_fh.readline()).strip()
    if line == '':
        break
