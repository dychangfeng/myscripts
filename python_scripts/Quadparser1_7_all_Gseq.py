...
re.compile(). The default regex is '([gG]{3,}\w{1,8}){3,}[gG]{3,}' which searches
for G-quadruplexes.
...
while True:
    ...
    for m in re.finditer(psq_re_f, ref_seq):
        ...
        if m.group(0).upper()[0] == 'C':
            gquad_list.append([chr, m.start(), m.end(), quad_id, len(m.group(0)), '+', reverse_comp(m.group(0).upper())])
        else:
            gquad_list.append([chr, m.start(), m.end(), quad_id, len(m.group(0)), '+', m.group(0).upper()])
