...
DESCRIPTION
    ...
    The defualt regex is '([cC]{3,})\w{1,12}([cC]{3,})\w{1,12}([cC]{3,})\w{1,12}([cC]{3,})' and its complement
...
for G-quadruplexes.
                   ...
                   ''',
                   default= '([cC]{3,})\w{1,12}([cC]{3,})\w{1,12}([cC]{3,})\w{1,12}([cC]{3,})')
...
while True:
    ...
    for m in re.finditer(psq_re_f, ref_seq):
        if len(m.group(1))==len(m.group(3)) and len(m.group(2)) == len(m.group(4)):
